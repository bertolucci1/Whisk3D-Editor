...
class DeleteUndo : public UndoCmd {
    ...
    // La ANIMACION de los borrados se la lleva ESTE comando, igual que los objetos. Tiene que ser asi: AnimationObjects
    // referencia al objeto POR PUNTERO, y cuando el comando muere hace delete del objeto -> el puntero queda colgando.
    // El proximo objeto que se cree puede caer EN ESA MISMA DIRECCION y "heredar" la animacion del muerto (paso:
    // animar un cubo, borrarlo, crear otro cubo y encontrarlo ya animado). Al deshacer vuelven con su objeto.
    ...
    // Se lleva del global la animacion de CADA objeto que este comando borra (y de todo su subarbol, que se va con
    // el). Vuelve en Reatachar.
        ...
        for (size_t i = 0; i < ents.size(); i++) {
            ...
            while (!pila.empty()) {
                ...
                for (size_t a = 0; a < AnimationObjects.size(); ) {
                    if (AnimationObjects[a].obj == o) {
                        anims.push_back(AnimationObjects[a]);
    ...
    void DevolverAnimaciones() {
        for (size_t i = 0; i < anims.size(); i++) AnimationObjects.push_back(anims[i]);
        anims.clear();
    }
